...
func NewCrawlerHandler(job *job.JobService, crawler *CrawlerService) *CrawlerHandler {
	return &CrawlerHandler{job: job, crawler: crawler}
}

func (h *CrawlerHandler) HandleCreateCrawl(c *fiber.Ctx) error {
	...
	id, err := h.crawler.Enqueue(c.Context(), req)
	if err != nil {
		errMsg := err.Error()
...
func (h *CrawlerHandler) HandleGetCrawl(c *fiber.Ctx) error {
	id := c.Params("jobId")
