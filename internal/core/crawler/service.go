...
func NewCrawlerService(job *job.JobService, tasks *tasks.Client, mapper *mapper.Service, scraper *scraper.ScraperService, cfg config.Config) *CrawlerService {
	return &CrawlerService{job: job, tasks: tasks, mapper: mapper, scraper: scraper, log: logger.New("CrawlerService"), config: cfg}
}

// StreamCrawlToChannel performs streaming crawl and publishes each page to the provided channel
func (s *CrawlerService) StreamCrawlToChannel(ctx context.Context, req engineapi.CrawlCreateRequest, pageChan chan<- *PageResult) {
...
func (s *CrawlerService) Enqueue(ctx context.Context, req engineapi.CrawlCreateRequest) (string, error) {
	id := uuid.New().String()
...
func (s *CrawlerService) HandleCrawlTask(ctx context.Context, task *asynq.Task) error {
...
func (s *CrawlerService) streamCrawl(ctx context.Context, r engineapi.CrawlCreateRequest) (*map[string]engineapi.PageContent, *map[string]string) {
...
func (s *CrawlerService) scrapeWithOptions(ctx context.Context, url string, includeHTML, renderJs, fresh bool, userAgent *string, waitSelectors *[]string) (*engineapi.ScrapeResponse, error) {
	if fresh {
		...
		return s.scraper.ScrapeURL(ctx, params)
	} else {
		// Use cache if available
		result, _, err := s.scraper.ScrapeWithCache(ctx, url, includeHTML, renderJs)
		return result, err
	}
}

// getLinks uses the scrape service to extract links from dynamic content with optional user agent and selectors
func (s *CrawlerService) getLinks(ctx context.Context, url string, renderJs bool, userAgent *string, waitSelectors *[]string) []string {
	...
	result, err := s.scraper.ScrapeURL(ctx, params)
	if err != nil {
...
func (s *CrawlerService) sendWebhookNotification(ctx context.Context, jobID, status string, data engineapi.CrawlJobData, webhookURL string, headers *map[string]string) {
...
func (s *CrawlerService) generateHMACSignature(timestamp string, payload []byte) string {
